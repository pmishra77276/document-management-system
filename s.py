...
# Models and globals placeholders
...
def load_embeddings_from_solr():
    ...
    try:
        ...
        resp1 = requests.get(BASE_URL, params={"q": "*:*", "wt": "json", "rows": total})
        resp1.raise_for_status()
        docs = resp1.json()["response"]["docs"]
        emb = {d['id']: np.array(d.get('total_embedding', []), dtype=float) for d in docs}
        emb.pop("/home/pa2/ml/Document-Classification/CLASSIFICATION FILES/11.Bill of Entry.txt", None)
        return emb
...
def initialize_models():
    global trocr_processor, trocr_model, detector, text_splitter, model, emb_dict, initialized
    if initialized:
        return
    ...
    model = HuggingFaceEmbeddings(model_name="intfloat/e5-large-v2")
